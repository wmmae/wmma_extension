...
__device__ inline float my_abs(float a) { return (a < 0) ? (-a) : a; }
...
__global__ void map_test(float *const error_ptr) {
  ...
  constexpr unsigned mat_m =
      mtk::wmma::detail::common::get_M<Use, M, N, K>::value;
  constexpr unsigned mat_n =
  ...
  for (unsigned i = 0; i < mat_m * mat_n; i += warp_size) {
    ...
    const auto m = std::is_same<Layout, nvcuda::wmma::row_major>::value
                       ? (index / mat_n)
                       : (index % mat_m);
    const auto n = std::is_same<Layout, nvcuda::wmma::row_major>::value
                       ? (index % mat_n)
                       : (index / mat_m);
  ...
  nvcuda::wmma::load_matrix_sync(
      frag_ref, smem,
      std::is_same<Layout, nvcuda::wmma::row_major>::value ? mat_n : mat_m);
    ...
    for (unsigned j = 0; j < mat_n; j++) {
      ...
      for (unsigned k = 0; k < list_size; k++) {
        if (threadIdx.x == tid_list[k]) {
          frag_map.x[fid_list[k]] =
              mtk::wmma::detail::common::cast<storage_t>(i + j * mat_m);
        }
        __syncwarp();
      }
    }
...
__global__ void map_test(float *const error_ptr,
  ...
  for (unsigned i = 0; i < mat_m * mat_n; i += warp_size) {
    const unsigned index = i + threadIdx.x;
    const float v = (layout == nvcuda::wmma::mem_row_major)
                        ? ((index / mat_n) + (index % mat_n) * mat_m)
                        : index;
    smem[index] = mtk::wmma::detail::common::cast<storage_t>(v);
  }
  for (unsigned i = 0; i < mat_m * mat_n; i += warp_size) {
    const unsigned index = i + threadIdx.x;
    const auto m = (layout == nvcuda::wmma::mem_row_major) ? (index / mat_n)
                                                           : (index % mat_m);
    const auto n = (layout == nvcuda::wmma::mem_row_major) ? (index % mat_n)
                                                           : (index / mat_m);
    smem[index] = mtk::wmma::detail::common::cast<storage_t>(m + n * mat_m);
  }
  ...
  nvcuda::wmma::load_matrix_sync(
      frag_ref, smem, (layout == nvcuda::wmma::mem_row_major) ? mat_n : mat_m,
      layout);
...
template <class Use, int M, int N, int K, class T, class Layout> void test() {
  ...
  std::printf("%s<%12s,%2d,%2d,%2d,%7s,%10s>:Error=%e [", __FILE__,
              mtk::test_utils::get_string<Use>().c_str(), M, N, K,
...
void test(const nvcuda::wmma::layout_t layout) {
  ...
  std::printf(
      "%s<%12s,%2d,%2d,%2d,%7s,%10s>:Error=%e [", __FILE__,
      mtk::test_utils::get_string<Use>().c_str(), M, N, K,
      mtk::test_utils::get_string<T>().c_str(),
      (layout == nvcuda::wmma::mem_col_major)
          ? mtk::test_utils::get_string<nvcuda::wmma::col_major>().c_str()
          : mtk::test_utils::get_string<nvcuda::wmma::row_major>().c_str(),
      *error);
  if (*error < 1) {
    std::printf("PASSED");
  } else {
...
int main() {
  ...
  test<nvcuda::wmma::accumulator, 16, 16, 16, half>(
      nvcuda::wmma::mem_col_major);
  test<nvcuda::wmma::accumulator, 16, 16, 16, float>(
      nvcuda::wmma::mem_col_major);
  test<nvcuda::wmma::accumulator, 16, 16, 16, half>(
  ...
  test<nvcuda::wmma::matrix_b, 16, 16, 8, nvcuda::wmma::precision::tf32,
       nvcuda::wmma::row_major>();
  test<nvcuda::wmma::accumulator, 16, 16, 8, float>(
      nvcuda::wmma::mem_col_major);
  test<nvcuda::wmma::accumulator, 16, 16, 8, float>(
      nvcuda::wmma::mem_row_major);
#endif
