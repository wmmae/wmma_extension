...
template <class Use, int M, int N, int K, class Type, class Layout,
          unsigned MATRIX_DIM>
__global__ void test_kernel(float *const diff, const float *const src,
                            const unsigned ld) {
  ...
  mtk::wmma::foreach_ij<decltype(frag_mtk)>(
      [&](const unsigned *frag_index_list, const unsigned num_indeces,
...
template <class Use, int M, int N, int K, class Type,
          nvcuda::wmma::layout_t layout, unsigned MATRIX_DIM>
__global__ void test_kernel_acc(float *const diff, const float *const src,
                                const unsigned ld) {
  using storage_t = typename mtk::wmma::detail::common::storage_t<Type>::type;
  ...
  mtk::wmma::foreach_ij<decltype(frag_mtk)>(
      layout, [&](const unsigned *frag_index_list, const unsigned num_indeces,
                  const unsigned i, const unsigned j) {
        unsigned mem_index;
...
void test() {
  ...
  test_kernel<Use, M, N, K, Type, Layout, MATRIX_DIM>
      <<<1, warp_size>>>(diff, src_matrix, MATRIX_DIM);
  ...
  std::printf("%s{SM=%2d,Use=%15s,M=%2d,N=%2d,K=%2d,Type=%5s,Layout=%8s}:",
              __FILE__, TEST_ARCH, mtk::test_utils::get_string<Use>().c_str(),
              M, N, K, mtk::test_utils::get_string<Type>().c_str(),
...
void test_acc() {
  ...
  if (std::is_same<Layout, nvcuda::wmma::col_major>::value) {
    test_kernel_acc<Use, M, N, K, Type, nvcuda::wmma::mem_col_major, MATRIX_DIM>
        <<<1, warp_size>>>(diff, src_matrix, MATRIX_DIM);
  } else {
    test_kernel_acc<Use, M, N, K, Type, nvcuda::wmma::mem_row_major, MATRIX_DIM>
        <<<1, warp_size>>>(diff, src_matrix, MATRIX_DIM);
  }
  cudaDeviceSynchronize();
...
int main() {
  ...
  test_acc<nvcuda::wmma::accumulator, 16, 16, 16, half,
           nvcuda::wmma::col_major>();
  test_acc<nvcuda::wmma::accumulator, 16, 16, 16, float,
           nvcuda::wmma::col_major>();
  test_acc<nvcuda::wmma::accumulator, 16, 16, 16, half,
  ...
  test<nvcuda::wmma::matrix_b, 16, 16, 8, nvcuda::wmma::precision::tf32,
       nvcuda::wmma::col_major>();
  test_acc<nvcuda::wmma::accumulator, 16, 16, 8, float,
           nvcuda::wmma::col_major>();
  test_acc<nvcuda::wmma::accumulator, 16, 16, 8, float,
           nvcuda::wmma::row_major>();
#endif
