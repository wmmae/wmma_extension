...
__global__ void matmul16x16_kernel(float *const c_ptr, const float *const a_ptr,
  ...
  mtk::wmma::foreach<decltype(frag_a)>([&](const unsigned frag_index_list[],
                                           const unsigned frag_index_count,
                                           const unsigned mem_index) {
    const auto a = a_ptr[mem_index];
    const auto a_rp = mtk::wmma::detail::common::cast<ab_type>(a);
    const auto da_rp = mtk::wmma::detail::common::cast<ab_type>(
        a - mtk::wmma::detail::common::cast<float>(a_rp));
    for (unsigned i = 0; i < frag_index_count; i++) {
      const unsigned frag_index = frag_index_list[i];
  ...
  mtk::wmma::foreach<decltype(frag_b)>([&](const unsigned frag_index_list[],
                                           const unsigned frag_index_count,
                                           const unsigned mem_index) {
    const auto b = b_ptr[mem_index];
    const auto b_rp = mtk::wmma::detail::common::cast<ab_type>(b);
    const auto db_rp = mtk::wmma::detail::common::cast<ab_type>(
        b - mtk::wmma::detail::common::cast<float>(b_rp));
    for (unsigned i = 0; i < frag_index_count; i++) {
      const unsigned frag_index = frag_index_list[i];
  ...
  nvcuda::wmma::store_matrix_sync(c_ptr, frag_c, N,
                                  nvcuda::wmma::mem_col_major);
}
...
void test() {
  ...
  for (unsigned i = 0; i < M; i++) {
    for (unsigned j = 0; j < N; j++) {
      ...
      for (unsigned k = 0; k < K; k++) {
        sum += static_cast<double>(a[i + M * k]) *
               static_cast<double>(b[k + j * K]);
      }
  ...
  std::printf("{%s} error=%e [", __FILE__, e);
  if (e < 1e-6) {
    std::printf("PASSED");
  } else {
    std::printf("FAILED");
