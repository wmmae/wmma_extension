...
__global__ void test_gevm_kernel(float *const dst, const half *const src,
...
void test() {
  ...
  for (std::size_t i = 0; i < 16; i++) {
    const double diff =
        convert<float, half>(dst_mem[i]) - convert<float, half>(src_mem[i]);
    error = std::max(error, std::abs(diff));
  }
  std::printf("[%s] ARCH=%d, error=%e [%s]\n", __FILE__, TEST_ARCH, error,
              mtk::test_utils::get_test_result_string(
                  error < mtk::test_utils::get_machine_eps<float>() * 16));
}
