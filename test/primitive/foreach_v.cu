...
__device__ __host__ typename mtk::wmma::detail::common::storage_t<T>::type
convert(const S);
template <>
__device__ __host__ typename mtk::wmma::detail::common::storage_t<float>::type
convert<float, float>(const float a) {
  return a;
}
template <>
__device__ __host__ typename mtk::wmma::detail::common::storage_t<float>::type
convert<float, half>(const half a) {
  return __half2float(a);
}
template <>
__device__ __host__ typename mtk::wmma::detail::common::storage_t<half>::type
convert<half, float>(const float a) {
  return __float2half(a);
}
template <>
__device__ __host__ typename mtk::wmma::detail::common::storage_t<half>::type
convert<half, half>(const half a) {
  return a;
}
...
__global__ void test_foreach_v_kernel(const storage_t *const src,
                                      const storage_t *const cor) {
  nvcuda::wmma::fragment<Use, M, N, K, ab_type, layout> vec_frag;
  mtk::wmma::fill_zero(vec_frag);
  mtk::wmma::foreach_v<decltype(vec_frag)>([&](const unsigned frag_index_list[],
...
template <class Use, class layout> void test() {
  std::printf("-- test (%s) --\n", __FILE__);
