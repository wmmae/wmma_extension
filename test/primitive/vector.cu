...
template <class T> __device__ float error_threshold();
template <> __device__ float error_threshold<float>() { return 1e-6f; };
template <> __device__ float error_threshold<half>() { return 1e-3f; };
...
__global__ void test_load_vector_ab_kernel(
  ...
  mtk::wmma::mma::fragment<Use, m, n, k, T,
...
__global__ void test_load_vector_acc_kernel(const T *const src,
                                            const T *const cor) {
  ...
  if (std::is_same<Layout, nvcuda::wmma::col_major>::value) {
    ...
    mtk::wmma::mma::load_matrix_sync(cor_frag, cor, n,
