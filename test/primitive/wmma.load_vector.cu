...
template <class T, class S> __device__ __host__ T convert(const S);
template <> __device__ __host__ float convert<float, float>(const float a) {
  return a;
}
template <> __device__ __host__ float convert<float, half>(const half a) {
  return __half2float(a);
}
template <> __device__ __host__ half convert<half, float>(const float a) {
  return __float2half(a);
}
template <> __device__ __host__ half convert<half, half>(const half a) {
  return a;
}

template <class T> __device__ T m_abs(const T a) {
  if (a >= convert<T, float>(0))
    return a;
  return -a;
}

template <class Use, class Layout> struct fragment_layout {
  using type = Layout;
};
template <>
struct fragment_layout<nvcuda::wmma::accumulator, nvcuda::wmma::col_major> {
  using type = void;
};
template <>
struct fragment_layout<nvcuda::wmma::accumulator, nvcuda::wmma::row_major> {
  using type = void;
};
...
__global__ void test_load_vector_ab_kernel(
  ...
  nvcuda::wmma::fragment<Use, m, n, k, T,
                         typename fragment_layout<Use, Layout>::type>
      cor_frag;
  nvcuda::wmma::load_matrix_sync(cor_frag, cor, m);

  auto e =
      convert<typename mtk::wmma::detail::common::storage_t<T>::type, float>(
          0.0f);
...
__global__ void test_load_vector_acc_kernel(float *const error,
                                            const T *const src,
                                            const T *const cor) {
...
template <class Use, int m, int n, int k, class T, class Layout> void test() {
  ...
  if constexpr (std::is_same<Use, nvcuda::wmma::accumulator>::value) {
    test_load_vector_acc_kernel<m, n, k, T, Layout>
        <<<1, 32>>>(error, src_mem, cor_mem);
  } else {
    test_load_vector_ab_kernel<Use, m, n, k, T, Layout>
        <<<1, 32>>>(error, src_mem, cor_mem);
  }
  cudaDeviceSynchronize();
  std::printf("[%s] ARCH=%d, <%2d, %2d, %2d>, %10s, %10s, error=%e [%s]\n",
              ...
              mtk::test_utils::get_test_result_string(
                  (*error) < mtk::test_utils::get_machine_eps<T>() * 16));
  cudaFreeHost(error);
}
