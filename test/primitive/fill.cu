...
__device__ float my_fabs(const float a) { return a > 0.f ? a : -a; }

template <class Use, int M, int N, int K, class T, class Layout>
__global__ void fill_test_kernel(float *const g_max_error_a,
                                 float *const g_max_error_z) {
...
template <class Use, int M, int N, int K, class T, class Layout> void test() {
  ...
  fill_test_kernel<Use, M, N, K, T, Layout>
      <<<1, 32>>>(max_error_a, max_error_z);
  cudaDeviceSynchronize();
  std::printf("[%s] ARCH=%d, %11s, %2d, %2d, %2d, %5s, %10s, "
              "fill_zero_error=%e [%s], fill_a_error=%e, [%s]\n",
              __FILE__, TEST_ARCH, mtk::test_utils::get_string<Use>().c_str(),
              M, N, K, mtk::test_utils::get_string<T>().c_str(),
              mtk::test_utils::get_string<Layout>().c_str(), *max_error_a,
              mtk::test_utils::get_test_result_string(
                  (*max_error_a) < mtk::test_utils::get_machine_eps<T>()),
              *max_error_z,
              mtk::test_utils::get_test_result_string(
                  (*max_error_z) < mtk::test_utils::get_machine_eps<T>()));
