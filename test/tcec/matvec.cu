...
__global__ void matvec_kernel(float *const y_ptr, const float *const a_ptr,
                              const float *const x_ptr) {
  __shared__ float smem[N * N];
  mtk::test_utils::fill_zero(smem, N * N);

  mtk::wmma::tcec::fragment<nvcuda::wmma::matrix_a, N, N, N, T,
                            nvcuda::wmma::col_major, Policy>
      frag_a;
  mtk::wmma::tcec::fragment<nvcuda::wmma::matrix_b, N, N, N, T,
                            nvcuda::wmma::col_major, Policy>
      frag_x;
...
template <unsigned N, class T, class Policy> void test_matvec() {
  float *hX, *hY, *hA;
  ...
  std::printf(
      ...
      std::is_same<typename Policy::error_correction,
          ...
          ? "{w/ ec}"
          : "{w/o ec}",
      Policy::m, Policy::n, Policy::k, max_error,
...
#ifdef TEST_TF32
  // wmma TF32 test
  test_matvec<32, nvcuda::wmma::precision::tf32,
              typename mtk::wmma::tcec::detail::default_policy<
