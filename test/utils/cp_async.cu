...
template <class T> __host__ __device__ constexpr unsigned get_size_in_byte();
template <> __host__ __device__ constexpr unsigned get_size_in_byte<float>() {
  return 4;
};
template <> __host__ __device__ constexpr unsigned get_size_in_byte<float2>() {
  return 8;
};
template <> __host__ __device__ constexpr unsigned get_size_in_byte<float4>() {
  return 16;
};

template <class T, unsigned block_size>
__global__ void cp_async_test_kernel(T *const dst_ptr, const T *const src_ptr) {
  __shared__ T smem[block_size];

  mtk::wmma::utils::cp_async::cp_async<get_size_in_byte<T>()>(
      smem + threadIdx.x, src_ptr + threadIdx.x);
...
template <class T, unsigned block_size> void cp_async_test() {
  T *d_input;
  T *d_output;
  T *h_input;
  T *h_output;
  ...
  double max_error = 0;
  for (unsigned i = 0; i < block_size * get_size_in_byte<T>() / 4; i++) {
    const double diff = reinterpret_cast<float *>(h_output)[i] -
                        reinterpret_cast<float *>(h_input)[i];
    max_error = std::max(std::abs(diff), max_error);
  }

  std::printf("%s[%2u Byte] error = %e\n", __func__, get_size_in_byte<T>(),
              max_error);
