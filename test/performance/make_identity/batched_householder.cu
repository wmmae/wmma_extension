...
template <>
__global__ void householder_16x16<true>(float *const c_ptr,
                                        const half *const u_ptr) {
  ...
  nvcuda::wmma::store_matrix_sync(c_smem_ptr, c_frag[0], DIM,
                                  nvcuda::wmma::mem_col_major);
  nvcuda::wmma::store_matrix_sync(c_smem_ptr + FDIM, c_frag[1], DIM,
                                  nvcuda::wmma::mem_col_major);
  nvcuda::wmma::store_matrix_sync(c_smem_ptr + FDIM * DIM, c_frag[2], DIM,
                                  nvcuda::wmma::mem_col_major);
  nvcuda::wmma::store_matrix_sync(c_smem_ptr + FDIM * DIM + FDIM, c_frag[3],
                                  DIM, nvcuda::wmma::mem_col_major);
...
template <>
__global__ void householder_16x16<false>(float *const c_ptr,
                                         const half *const u_ptr) {
  constexpr unsigned DIM = 32;
  constexpr unsigned FDIM = 16;
  __shared__ half u_smem[block_size];
  ...
  __syncthreads();
  nvcuda::wmma::load_matrix_sync(c_frag[0], c_smem_ptr, FDIM,
                                 nvcuda::wmma::mem_col_major);
  nvcuda::wmma::load_matrix_sync(c_frag[3], c_smem_ptr, FDIM,
                                 nvcuda::wmma::mem_col_major);
...
template <bool UseWMMAe> void test_householder(const unsigned size_power) {
  ...
  for (std::size_t c = 0; c < C; c++) {
    householder_16x16<UseWMMAe><<<grid_size, block_size>>>(dC, dU);
  }
  ...
  std::printf("%u,%u,%u,%e\n", static_cast<unsigned>(CUDA_ARCH_SM), batch_size,
...
int main() { test_householder(8, 14); }
