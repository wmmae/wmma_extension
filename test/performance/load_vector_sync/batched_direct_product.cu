...
__global__ void batched_direct_product_16x16(float *const c_ptr,
                                             const half *const u_ptr);

template <>
__global__ void batched_direct_product_16x16<true>(float *const c_ptr,
                                                   const half *const u_ptr) {
...
template <>
__global__ void batched_direct_product_16x16<false>(float *const c_ptr,
                                                    const half *const u_ptr) {
  constexpr unsigned DIM = 32;
  constexpr unsigned FDIM = 16;
...
void test_batched_direct_product(const unsigned size_power) {
  ...
  for (std::size_t c = 0; c < C; c++) {
    batched_direct_product_16x16<UseWMMAe><<<grid_size, block_size>>>(dC, dU);
  }
...
int main() { test_batched_direct_product(8, 20); }
