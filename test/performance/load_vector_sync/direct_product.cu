...
__global__ void direct_product(float *const c_ptr, const half *const a_ptr,
                               const half *const b_ptr, unsigned dim);

template <>
__global__ void direct_product<true>(float *const c_ptr,
  ...
  for (unsigned b_start = 0; b_start < dim; b_start += block_size) {
    ...
    nvcuda::wmma::store_matrix_sync(C_smem_ptr, C_frag[0], warp_size,
                                    nvcuda::wmma::mem_col_major);
    nvcuda::wmma::mma_sync(C_frag[1], A_frag[1], B_frag[0], C_frag[1]);
    nvcuda::wmma::store_matrix_sync(C_smem_ptr + FDIM, C_frag[1], warp_size,
                                    nvcuda::wmma::mem_col_major);

    mtk::wmma::load_vector(B_frag[1], B_smem + warp_size * warp_id + FDIM,
                           false);
    nvcuda::wmma::mma_sync(C_frag[2], A_frag[0], B_frag[1], C_frag[2]);
    nvcuda::wmma::store_matrix_sync(C_smem_ptr + warp_size * FDIM, C_frag[2],
                                    warp_size, nvcuda::wmma::mem_col_major);
    nvcuda::wmma::mma_sync(C_frag[3], A_frag[1], B_frag[1], C_frag[3]);
    nvcuda::wmma::store_matrix_sync(C_smem_ptr + FDIM + warp_size * FDIM,
                                    C_frag[3], warp_size,
                                    nvcuda::wmma::mem_col_major);

    for (unsigned i = 0; i < block_size * warp_size; i += block_size) {
      c_ptr[a_start + unique_id + (warp_id + i / warp_size + b_start) * dim] =
          C_smem[i + threadIdx.x];
    }
  }
}

template <>
__global__ void direct_product<false>(float *const c_ptr,
                                      const half *const a_ptr,
                                      const half *const b_ptr, unsigned dim) {
  constexpr unsigned FDIM = 16;
  ...
  for (unsigned b_start = 0; b_start < dim; b_start += block_size) {
    ...
    nvcuda::wmma::load_matrix_sync(B_frag[0], B_smem + warp_size * warp_id,
                                   block_size);
    nvcuda::wmma::mma_sync(C_frag[0], A_frag[0], B_frag[0], C_frag[0]);
    nvcuda::wmma::store_matrix_sync(C_smem_ptr, C_frag[0], warp_size,
                                    nvcuda::wmma::mem_col_major);
    ...
    nvcuda::wmma::load_matrix_sync(
        B_frag[1], B_smem + warp_size * warp_id + FDIM, block_size);
    nvcuda::wmma::mma_sync(C_frag[2], A_frag[0], B_frag[1], C_frag[2]);
    nvcuda::wmma::store_matrix_sync(C_smem_ptr + warp_size * FDIM, C_frag[2],
                                    warp_size, nvcuda::wmma::mem_col_major);
    nvcuda::wmma::mma_sync(C_frag[3], A_frag[1], B_frag[1], C_frag[3]);
...
template <bool UseWMMAe> void test_direct_product(const unsigned size_power) {
  ...
  std::printf("%u,%u,%u,%e,%e,%e\n", static_cast<unsigned>(CUDA_ARCH_SM), DIM,
...
int main() { test_direct_product(8, 15); }
