...
__device__ void cp_matrix(half2 *const smem, const half2 *const gmem) {
  for (unsigned i = 0; i < DIM * DIM / 2; i += warp_size * 4) {
    ...
    reinterpret_cast<uint4 *>(smem)[index] =
        reinterpret_cast<const uint4 *>(gmem)[index];
  }
}

template <unsigned DIM, class GivensMatGen>
__global__ void batched_givens_kernel(half *const ptr,
  ...
  nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, DIM, DIM, DIM, half,
  ...
  nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, DIM, DIM, DIM, half,
                         nvcuda::wmma::col_major>
      frag_b;
  ...
  nvcuda::wmma::store_matrix_sync(smem_ptr, frag_c, DIM,
...
template <unsigned DIM, class GivensMatGen>
__global__ void batched_givens_kernel(half *const ptr, const unsigned gi,
                                      const unsigned gj,
                                      const unsigned batch_size) {
  __shared__ half smem[DIM * DIM * block_size / warp_size];
...
template <unsigned DIM> struct GivensMatGenWMMA {
  __device__ GivensMatGenWMMA() {}
  __device__ void
  operator()(nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, DIM, DIM, DIM, half,
                                    nvcuda::wmma::col_major> &frag,
             const unsigned gi, const unsigned gj, const float theta,
             half *const smem) const {
...
template <unsigned DIM> struct GivensMatGenWMMAe {
...
template <> std::string get_class_name<GivensMatGenWMMA<16>>() {
  return "wmma_16";
}
template <> std::string get_class_name<GivensMatGenWMMAe<16>>() {
  return "wmmae_16";
}

template <unsigned DIM, class GivensMatGen>
void batched_givens(half *const ptr, const unsigned batch_size) {
  const unsigned grid_size =
      (batch_size * warp_size + block_size - 1) / block_size;
  batched_givens_kernel<DIM, GivensMatGen>
...
void test_batched_kernel(const unsigned batch_size) {
  ...
  for (unsigned c = 0; c < test_count; c++) {
    batched_givens<DIM, GivensMatGen>(input_matrix, batch_size);
  }
  ...
  std::printf("%u,1,%s,%e\n", batch_size,
              get_class_name<GivensMatGen>().c_str(), elapsed_time);
}

template <unsigned DIM, class GivensMatGen>
void batched_givens(half *const ptr, const unsigned gi, const unsigned gj,
                    const unsigned batch_size) {
  const unsigned grid_size =
      (batch_size * warp_size + block_size - 1) / block_size;
  batched_givens_kernel<DIM, GivensMatGen>
      <<<grid_size, block_size>>>(ptr, gi, gj, batch_size);
}

template <unsigned DIM, class GivensMatGen>
void test_batched_kernel(const unsigned gi, const unsigned gj,
                         const unsigned batch_size) {
  half *input_matrix;
  cudaMalloc(&input_matrix, sizeof(half) * DIM * DIM * batch_size);
  const auto start_clock = std::chrono::system_clock::now();
  for (unsigned c = 0; c < test_count; c++) {
    batched_givens<DIM, GivensMatGen>(input_matrix, gi, gj, batch_size);
  }
  cudaDeviceSynchronize();
  const auto end_clock = std::chrono::system_clock::now();
  cudaFree(input_matrix);
