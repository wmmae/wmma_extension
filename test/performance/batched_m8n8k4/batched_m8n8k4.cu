...
__global__ void batched_matmul_kernel(float *const c_ptr,
  ...
  mtk::wmma::fragment<nvcuda::wmma::matrix_a, M, N, K, half,
                      nvcuda::wmma::col_major>
      frag_a;
  mtk::wmma::fragment<nvcuda::wmma::matrix_b, M, N, K, half,
                      nvcuda::wmma::col_major>
      frag_b;
  ...
  mtk::wmma::store_matrix_sync(c_ptr + matrix_id * M * N, frag_c, M,
...
int main() {
  ...
  for (unsigned c = 0; c < C; c++)
    batched_matmul_kernel<<<
        (warp_size * num_matrices + block_size - 1) / block_size, block_size>>>(
        dc, da, db);
  ...
  std::printf("%15s : %e [TFlop/s]\n", "performance",
              (2 * M * N * K * C * num_matrices) / elapsed_time / (1lu << 40));
  std::printf("%15s : %e [GiB/s]\n", "band width",
              static_cast<std::size_t>(M * N * sizeof(float) +
                                       2 * (M * K + N * K) * sizeof(half)) *
                  num_matrices * C / elapsed_time / (1lu << 30));
