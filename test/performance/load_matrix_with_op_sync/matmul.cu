...
__global__ void matmul<true>(float *const c_ptr, const float *const a_ptr,
  ...
  for (unsigned kb = 0; kb < n; kb += block_size) {
    for (unsigned i = 0; i < block_size * warp_size; i += block_size) {
      const auto v =
          a_ptr[block_c_row + unique_id + (warp_id + i / warp_size + kb) * n];
    ...
    for (unsigned i = 0; i < 2; i++) {
      for (unsigned j = 0; j < 2; j++) {
        ...
        mtk::wmma::load_matrix_with_operation(
            ...
            [&](const unsigned index, const float v) {
              return __float2half(v - __half2float(frag_a[i + j * 2].x[index]));
            });
      }
    }
    ...
    for (unsigned i = 0; i < block_size * warp_size; i += block_size) {
      const auto v =
          b_ptr[(block_c_col + i / block_size) * n + kb + threadIdx.x];
    ...
    for (unsigned i = 0; i < 2; i++) {
      for (unsigned j = 0; j < 2; j++) {
        ...
        mtk::wmma::load_matrix_with_operation(
            ...
            [&](const unsigned index, const float v) {
              return __float2half(v - __half2float(frag_b[i + j * 2].x[index]));
            });
      }
    }
