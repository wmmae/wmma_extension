...
namespace {
template <class T> std::string get_name_str();
template <> std::string get_name_str<half>() { return "half"; }
template <> std::string get_name_str<float>() { return "float"; }
template <> std::string get_name_str<nvcuda::wmma::precision::tf32>() {
  return "tf32";
}
template <> std::string get_name_str<nvcuda::wmma::matrix_a>() {
  return "matrix_a";
}
template <> std::string get_name_str<nvcuda::wmma::matrix_b>() {
  return "matrix_b";
}
template <> std::string get_name_str<nvcuda::wmma::accumulator>() {
  return "accumulator";
}
template <> std::string get_name_str<nvcuda::wmma::col_major>() {
  return "col_major";
}
template <> std::string get_name_str<nvcuda::wmma::row_major>() {
  return "row_major";
}
...
__global__ void kernel(unsigned *const bank_array_ptr,
  ...
  const auto func = [&](const unsigned *frag_index_list,
                        const unsigned fragment_index_count, const unsigned i,
                        const unsigned j) {
    const unsigned mem_index =
        (layout == nvcuda::wmma::mem_col_major) ? (i + j * ldm) : (j + i * ldm);
...
template <class Use, int m, int n, int k, class T, class Layout>
void print_bank_conflict(const nvcuda::wmma::layout_t layout,
                         const std::size_t ldm) {
  const unsigned num_elements_per_thread =
      ((std::is_same<Use, nvcuda::wmma::matrix_a>::value) ? (m * k) : (k * n)) /
      warp_size;
  unsigned *bank_array;
  CUDA_CHECK_ERROR(cudaMallocHost(&bank_array, sizeof(unsigned) * bank_size *
                                                   num_elements_per_thread));
  for (unsigned i = 0; i < bank_size * num_elements_per_thread; i++)
    bank_array[i] = 0;
  std::printf("[<%s,%d,%d,%d,%s,%s>, layout = %s, ldm = %lu (skew=%lu)] "
              "---------------------------------------------------------- \n",
              get_name_str<Use>().c_str(), m, n, k, get_name_str<T>().c_str(),
              get_name_str<Layout>().c_str(),
...
int main() {
  for (std::size_t skew = skew_min; skew <= skew_max; skew++) {
    ...
    print_bank_conflict<nvcuda::wmma::matrix_a, 16, 8, 16, half,
                        nvcuda::wmma::row_major>(nvcuda::wmma::mem_col_major,
                                                 ldm);
    print_bank_conflict<nvcuda::wmma::matrix_a, 16, 8, 16, half,
                        nvcuda::wmma::row_major>(nvcuda::wmma::mem_row_major,
                                                 ldm);
    print_bank_conflict<nvcuda::wmma::matrix_b, 16, 8, 16, half,
                        nvcuda::wmma::col_major>(nvcuda::wmma::mem_col_major,
                                                 ldm);
    print_bank_conflict<nvcuda::wmma::matrix_b, 16, 8, 16, half,
                        nvcuda::wmma::col_major>(nvcuda::wmma::mem_row_major,
                                                 ldm);
    print_bank_conflict<nvcuda::wmma::accumulator, 16, 8, 16, float, void>(
        nvcuda::wmma::mem_col_major, ldm);
    print_bank_conflict<nvcuda::wmma::accumulator, 16, 8, 16, float, void>(
        nvcuda::wmma::mem_row_major, ldm);

    print_bank_conflict<nvcuda::wmma::matrix_a, 16, 8, 8,
                        nvcuda::wmma::precision::tf32, nvcuda::wmma::row_major>(
        nvcuda::wmma::mem_col_major, ldm);
    print_bank_conflict<nvcuda::wmma::matrix_a, 16, 8, 8,
                        nvcuda::wmma::precision::tf32, nvcuda::wmma::row_major>(
        nvcuda::wmma::mem_row_major, ldm);
    print_bank_conflict<nvcuda::wmma::matrix_b, 16, 8, 8,
                        nvcuda::wmma::precision::tf32, nvcuda::wmma::col_major>(
        nvcuda::wmma::mem_col_major, ldm);
    print_bank_conflict<nvcuda::wmma::matrix_b, 16, 8, 8,
                        nvcuda::wmma::precision::tf32, nvcuda::wmma::col_major>(
        nvcuda::wmma::mem_row_major, ldm);
    print_bank_conflict<nvcuda::wmma::accumulator, 16, 8, 8, float, void>(
        nvcuda::wmma::mem_col_major, ldm);
    print_bank_conflict<nvcuda::wmma::accumulator, 16, 8, 8, float, void>(
        nvcuda::wmma::mem_row_major, ldm);
  }
