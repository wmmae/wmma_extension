...
template <class Func>
__device__ inline void foreach(mtk::wmma::mma::fragment<nvcuda::wmma::matrix_b, 16, 8, 16, std::int8_t, nvcuda::wmma::col_major>& frag, Func func) {
	const unsigned col = mtk::wmma::detail::common::get_lane_id() / 4;
	const unsigned row = (mtk::wmma::detail::common::get_lane_id() % 4) * 4;
	for (unsigned i = 0; i < 4; i++) {
    {const unsigned frag_index_list[1] = {i};func(frag_index_list, 1, row + i + col * 16);}
  }
}

template <class Func>
__device__ inline void foreach(mtk::wmma::mma::fragment<nvcuda::wmma::accumulator, 16, 8, 16, std::int32_t>& frag, const nvcuda::wmma::layout_t layout, Func func) {
...
template <class Func>
__device__ inline void foreach_ij(mtk::wmma::mma::fragment<nvcuda::wmma::matrix_b, 16, 8, 16, std::int8_t, nvcuda::wmma::col_major>& frag, Func func) {
	const unsigned col = mtk::wmma::detail::common::get_lane_id() / 4;
	const unsigned row = (mtk::wmma::detail::common::get_lane_id() % 4) * 4;

	for (unsigned i = 0; i < 4; i++) {
    {const unsigned frag_index_list[1] = {i};func(frag_index_list, 1, row + i, col);}
  }
}

template <class Func>
__device__ inline void foreach_ij(mtk::wmma::mma::fragment<nvcuda::wmma::accumulator, 16, 8, 16, std::int32_t>& frag, const nvcuda::wmma::layout_t layout, Func func) {
	const unsigned col = (mtk::wmma::detail::common::get_lane_id() % 4) * 2;
	const unsigned row_offset = mtk::wmma::detail::common::get_lane_id() / 4;
...
__device__ inline void foreach(mtk::wmma::mma::fragment<nvcuda::wmma::matrix_a, 16, 8, 16, std::uint8_t, nvcuda::wmma::row_major>& frag, Func func) {
  foreach(*reinterpret_cast<mtk::wmma::mma::fragment<nvcuda::wmma::matrix_a, 16, 8, 16, std::int8_t, nvcuda::wmma::row_major>*>(&frag), func);
}
