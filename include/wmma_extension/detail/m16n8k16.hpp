...
namespace mma {
template <>
class fragment<nvcuda::wmma::matrix_a, 16, 8, 16, half, nvcuda::wmma::row_major>
    : public __frag_base<half, 8> {};
template <>
class fragment<nvcuda::wmma::matrix_b, 16, 8, 16, half, nvcuda::wmma::col_major>
    : public __frag_base<half, 4> {};
template <>
class fragment<nvcuda::wmma::accumulator, 16, 8, 16, float>
...
__device__ inline void foreach (
    mtk::wmma::mma::fragment<nvcuda::wmma::matrix_a, 16, 8, 16, half,
...
__device__ inline void foreach (
  ...
  for (unsigned i = 0; i < 2; i++) {
    const auto row = row_block_id * 2 + i * 8;
    {
      ...
      func(frag_index_list, 1, (row + 0) + col * 16);
    }
    {
      const unsigned frag_index_list[1] = {(i * 2 + 1)};
      func(frag_index_list, 1, (row + 1) + col * 16);
    }
  }
}
...
__device__ inline void foreach (
    mtk::wmma::mma::fragment<nvcuda::wmma::accumulator, 16, 8, 16, T> &frag,
...
foreach_ij(mtk::wmma::mma::fragment<nvcuda::wmma::matrix_a, 16, 8, 16, half,
                                    nvcuda::wmma::row_major> &frag,
           Func func) {
  const unsigned col_block_id =
      (mtk::wmma::detail::common::get_lane_id() % 4) * 2;
  const unsigned row_block_id = mtk::wmma::detail::common::get_lane_id() / 4;
...
template <class Func>
__device__ inline void
foreach_ij(mtk::wmma::mma::fragment<nvcuda::wmma::matrix_b, 16, 8, 16, half,
                                    nvcuda::wmma::col_major> &frag,
           Func func) {
  const unsigned col = mtk::wmma::detail::common::get_lane_id() / 4;
  const unsigned row_block_id =
      (mtk::wmma::detail::common::get_lane_id() % 4) * 2;
  ...
  for (unsigned i = 0; i < 2; i++) {
    ...
    {
      const unsigned frag_index_list[1] = {(i * 2 + 0)};
      func(frag_index_list, 1, row + 0, col);
    }
    {
      const unsigned frag_index_list[1] = {(i * 2 + 1)};
      func(frag_index_list, 1, row + 1, col);
    }
  }
}
...
template <class Func>
__device__ inline void
foreach_v(mtk::wmma::mma::fragment<nvcuda::wmma::matrix_a, 16, 8, 16, half,
                                   nvcuda::wmma::row_major> &frag,
          Func func) {
...

  {
    ...
    func(frag_index_list, 1, mtk::wmma::detail::common::get_lane_id() * 2 + 0);
  }
  {
    const unsigned frag_index_list[1] = {1};
    func(frag_index_list, 1, mtk::wmma::detail::common::get_lane_id() * 2 + 1);
  }
  {
    const unsigned frag_index_list[1] = {4};
    func(frag_index_list, 1, mtk::wmma::detail::common::get_lane_id() * 2 + 8);
  }
  {
    const unsigned frag_index_list[1] = {5};
    func(frag_index_list, 1, mtk::wmma::detail::common::get_lane_id() * 2 + 9);
  }
}

template <class Func>
__device__ inline void
foreach_v(mtk::wmma::mma::fragment<nvcuda::wmma::matrix_b, 16, 8, 16, half,
                                   nvcuda::wmma::col_major> &frag,
          Func func) {
  if (mtk::wmma::detail::common::get_lane_id() >= 4)
    return;
  ...
  if (layout == nvcuda::wmma::mem_col_major) {
      ...
      return;
    {
      ...
      func(frag_index_list, 1,
           mtk::wmma::detail::common::get_lane_id() / 4 + 0);
    }
    {
      const unsigned frag_index_list[1] = {2};
      func(frag_index_list, 1,
           mtk::wmma::detail::common::get_lane_id() / 4 + 8);
    }
  } else {
    if (mtk::wmma::detail::common::get_lane_id() >= 4)
      return;
    {
      const unsigned frag_index_list[1] = {0};
      func(frag_index_list, 1,
           mtk::wmma::detail::common::get_lane_id() * 2 + 0);
    }
    {
      const unsigned frag_index_list[1] = {1};
      func(frag_index_list, 1,
           mtk::wmma::detail::common::get_lane_id() * 2 + 1);
    }
  }
}
...
// Mma
__device__ inline void
mma_sync(fragment<nvcuda::wmma::accumulator, 16, 8, 16, float> &d,
         const fragment<nvcuda::wmma::matrix_a, 16, 8, 16, half,
                        nvcuda::wmma::row_major> &a,
         const fragment<nvcuda::wmma::matrix_b, 16, 8, 16, half,
...
__device__ inline void
mma_sync(fragment<nvcuda::wmma::accumulator, 16, 8, 16, half> &d,
         const fragment<nvcuda::wmma::matrix_a, 16, 8, 16, half,
                        nvcuda::wmma::row_major> &a,
         const fragment<nvcuda::wmma::matrix_b, 16, 8, 16, half,
                        nvcuda::wmma::col_major> &b,
