...
__device__ inline void foreach (
  ...
  const unsigned start_index = (((lane_id >> 2) & 0b1) << 3) +
                               ((lane_id >> 4) << 2) + ((lane_id & 0b11) << 4);
...
__device__ inline void foreach (
    ...
    Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  const unsigned start_index = (((lane_id >> 2) & 0b1) << 7) +
                               ((lane_id >> 4) << 6) + ((lane_id & 0b11) << 4);
  for (unsigned x = 0; x < frag.num_elements; x++) {
...
__device__ inline void foreach (
    ...
    Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  const unsigned start_index = (((lane_id >> 3) & 0b1) << 7) +
                               ((lane_id >> 4) << 6) + ((lane_id & 0b11) << 4);
  for (unsigned x = 0; x < frag.num_elements; x++) {
    const unsigned offset = x;
    const unsigned index = start_index + offset;
...
__device__ inline void foreach (
    ...
    Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  const unsigned start_index = (((lane_id >> 3) & 0b1) << 3) +
                               ((lane_id >> 4) << 2) + ((lane_id & 0b11) << 4);
  for (unsigned x = 0; x < frag.num_elements; x++) {
    const unsigned offset = ((x >> 2) << 6) + (x & 0b11);
    const unsigned index = start_index + offset;
...
__device__ inline void foreach (
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, half, void>
        &frag,
    const nvcuda::wmma::layout_t layout, Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  if (layout == nvcuda::wmma::mem_row_major) {
    const unsigned start_index =
        (lane_id & 0b11) * 16 + ((lane_id >> 2) & 0b1) * 128 +
        ((lane_id >> 3) & 0b1) * 8 + ((lane_id >> 4) & 0b1) * 8;
    for (unsigned x = 0; x < frag.num_elements; x++) {
      const unsigned frag_index_list[1] = {x};
      func(frag_index_list, 1, start_index + x);
    }
  } else {
    const unsigned start_index = (lane_id & 0b11) + ((lane_id >> 2) & 0b1) * 8 +
                                 ((lane_id >> 3) & 0b1) * 128 +
                                 ((lane_id >> 4) & 0b1) * 4;
    for (unsigned x = 0; x < frag.num_elements; x++) {
      const unsigned frag_index_list[1] = {x};
...
__device__ inline void foreach (
  ...
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  if (layout == nvcuda::wmma::mem_row_major) {
    const unsigned start_index = (lane_id & 0b1) + ((lane_id >> 1) & 0b1) * 32 +
                                 ((lane_id >> 2) & 0b1) * 8 +
                                 ((lane_id >> 3) & 0b1) * 128 +
                                 ((lane_id >> 4) & 0b1) * 4;
    for (unsigned x = 0; x < frag.num_elements; x++) {
      const unsigned frag_index_list[1] = {x};
      func(frag_index_list, 1,
           start_index + (x & 0b1) * 16 + ((x >> 1) & 0b1) * 2 + (x >> 2) * 64);
    }
  } else {
    const unsigned start_index =
        (lane_id & 0b1) + (lane_id & 0b10) * 16 + (lane_id & 0b100) * 2 +
        (lane_id & 0b1000) * 16 + ((lane_id & 0b10000) >> 2);
    for (unsigned x = 0; x < frag.num_elements; x++) {
      const unsigned frag_index_list[1] = {x};
      func(frag_index_list, 1,
           start_index + (x & 0b1) * 16 + (x & 0b10) + (x & 0b100) * 16);
    }
  }
}
...
foreach_ij(nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 16, half,
                                  nvcuda::wmma::col_major> &frag,
           Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
...
template <class Func>
__device__ inline void
foreach_ij(nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 16, 16, 16, half,
                                  nvcuda::wmma::col_major> &frag,
           Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  const auto i_offset = 0;
  const auto j_offset =
      (lane_id & 0b11) + (lane_id & 0b1000) + (lane_id & 0b10000) / 4;
  for (unsigned x = 0; x < frag.num_elements; x++) {
...
foreach_ij(nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 16, half,
  ...
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  const auto i_offset =
      (lane_id & 0b11) + (lane_id & 0b100) * 2 + (lane_id & 0b10000) / 4;
...
template <class T, class Func>
__device__ inline void
foreach_ij(nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, half,
                                  void> &frag,
           const nvcuda::wmma::layout_t layout, Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  const unsigned row = (lane_id & 0b11) + ((lane_id >> 2) & 0b1) * 8 +
                       ((lane_id >> 4) & 0b1) * 4;
  for (unsigned x = 0; x < frag.num_elements; x++) {
...
template <class T, class Func>
__device__ inline void
foreach_ij(nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, float,
                                  void> &frag,
           const nvcuda::wmma::layout_t layout, Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  const unsigned row_start =
      (lane_id & 0b1) + ((lane_id >> 2) & 0b1) * 8 + ((lane_id >> 4) & 0b1) * 4;
  const unsigned col_start =
...
template <class Func>
__device__ inline void
foreach_v(nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 16, half,
                                 nvcuda::wmma::col_major> &frag,
          Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  const unsigned long index_offset =
      ((lane_id >> 4) << 2) + (((lane_id >> 2) & 0x1) << 3);
...
template <class Func>
__device__ inline void
foreach_v(nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 16, 16, 16, half,
                                 nvcuda::wmma::col_major> &frag,
          Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
...
__device__ inline void foreach_v(
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, half> &frag,
    const nvcuda::wmma::layout_t layout, Func func) {
  const auto tid = threadIdx.x & 0x1f;
  if (layout == nvcuda::wmma::mem_col_major) {
    if (!(tid & 0b01000)) {
      const auto mem_index =
          ((tid & 0b10000) >> 2) + (tid & 0x3) + ((tid & 0x4) << 1);
      {
        const unsigned frag_index_list[1] = {0};
        func(frag_index_list, 1, mem_index + 0);
      }
    }
  } else {
    if (tid == 0 || tid == 8) {
      {
        const unsigned frag_index_list[1] = {0};
        func(frag_index_list, 1, tid + 0);
      }
      {
        const unsigned frag_index_list[1] = {1};
        func(frag_index_list, 1, tid + 1);
      }
      {
        const unsigned frag_index_list[1] = {2};
        func(frag_index_list, 1, tid + 2);
      }
      {
        const unsigned frag_index_list[1] = {3};
        func(frag_index_list, 1, tid + 3);
      }
      {
        const unsigned frag_index_list[1] = {4};
        func(frag_index_list, 1, tid + 4);
      }
      {
        const unsigned frag_index_list[1] = {5};
        func(frag_index_list, 1, tid + 5);
      }
      {
        const unsigned frag_index_list[1] = {6};
        func(frag_index_list, 1, tid + 6);
      }
      {
        const unsigned frag_index_list[1] = {7};
        func(frag_index_list, 1, tid + 7);
      }
    }
  }
}
...
template <class Func>
__device__ inline void foreach_v(
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, float> &frag,
    const nvcuda::wmma::layout_t layout, Func func) {
  const auto tid = threadIdx.x & 0x1f;
  if (layout == nvcuda::wmma::mem_col_major) {
    if (!(tid & 0b10) && !(tid & 0b1000)) {
      const auto mem_index =
          ((tid & 0b10000) >> 2) + (tid & 0b1) + ((tid & 0b100) << 1);
      {
        const unsigned frag_index_list[1] = {0};
        func(frag_index_list, 1, mem_index + 0);
      }
      {
        const unsigned frag_index_list[1] = {2};
        func(frag_index_list, 1, mem_index + 2);
      }
    }
  } else {
...
// map function
__device__ inline void
map(nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 16, half,
                           nvcuda::wmma::col_major> &frag,
    unsigned tid_list[2], unsigned fid_list[2], unsigned &list_size,
...
__device__ inline void
map(nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 16, 16, 16, half,
                           nvcuda::wmma::col_major> &frag,
    unsigned tid_list[2], unsigned fid_list[2], unsigned &list_size,
    const unsigned i, const unsigned j) {
...
__device__ inline void
map(nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, half, void>
        &frag,
    unsigned tid_list[2], unsigned fid_list[2], unsigned &list_size,
    const unsigned i, const unsigned j) {
  const auto tid_head =
      (i & 0b11) + (i & 0b100) * 4 + (i & 0b1000) / 2 + (j & 0b1000);
...
__device__ inline void
map(nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, float, void>
        &frag,
    unsigned tid_list[2], unsigned fid_list[2], unsigned &list_size,
    const unsigned i, const unsigned j) {
  const auto tid_head =
      (i & 0b1) + (i & 0b100) * 4 + (i & 0b1000) / 2 + (j & 0b1010);
...
__device__ inline void make_identity_matrix(
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, T> &frag) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
...
__device__ inline void make_direct_product_fragment(
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 16, half,
                           nvcuda::wmma::col_major> &frag_a,
    const T *const a, const S *const da, const bool fill) {
  if (fill) {
...
__device__ inline void make_direct_product_fragment(
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 16, 16, 16, half,
                           nvcuda::wmma::row_major> &frag_b,
    const T *const b, const S *const db, const bool fill) {
  if (fill) {
...
__device__ inline void make_direct_product_fragment(
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 16, half,
                           nvcuda::wmma::col_major> &frag_a,
    const float *const a_ptr, const bool fill) {
  if (fill) {
    mtk::wmma::fill_zero(frag_a);
  }
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
...
#pragma unroll
  for (unsigned i = 0; i < 4; i++) {
    ...
    if (is_residual)
      frag_a.x[i] = detail::common::cast<half>(
          a_fp32 - detail::common::cast<float>(frag_a.x[i]));
  }
}
...
__device__ inline void make_direct_product_fragment(
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 16, 16, 16, half,
                           nvcuda::wmma::row_major> &frag_b,
    const float *const b_ptr, const bool fill) {
  if (fill) {
    mtk::wmma::fill_zero(frag_b);
  }
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
...
#pragma unroll
  for (unsigned i = 0; i < 4; i++) {
    ...
    if (is_residual)
      frag_b.x[i] = detail::common::cast<half>(
          b_fp32 - detail::common::cast<float>(frag_b.x[i]));
  }
}
} // namespace sm_70
} // namespace detail
