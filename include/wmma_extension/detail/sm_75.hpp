...
__device__ inline void foreach (
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, T, void>
        &frag,
  ...
  if (layout == nvcuda::wmma::mem_col_major) {
    ...
    for (unsigned i = 0; i < frag.num_elements; i++) {
      const unsigned index = start_index + (i & 0b1) * 16 +
                             ((i >> 1) & 0b1) * 8 + ((i >> 2) & 0b1) * 128;
      const unsigned frag_index_list[1] = {i};
      func(frag_index_list, 1, index);
    }
  } else {
    const unsigned start_index =
        (lane_id & 0b11) * 2 + ((lane_id >> 2) & 1) * 16 +
        ((lane_id >> 3) & 0b1) * 32 + ((lane_id >> 4) & 0b1) * 64;
    for (unsigned i = 0; i < frag.num_elements; i++) {
      const unsigned index = start_index + (i & 0b1) + ((i >> 1) & 0b1) * 128 +
                             ((i >> 2) & 0b1) * 8;
      const unsigned frag_index_list[1] = {i};
      func(frag_index_list, 1, index);
    }
...
template <class T, class Func>
__device__ inline void
foreach_ij(nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, T,
                                  void> &frag,
           const nvcuda::wmma::layout_t layout, Func func) {
  const unsigned lane_id = mtk::wmma::detail::common::get_lane_id();
  ...
  if (layout == nvcuda::wmma::mem_col_major) {
    if ((tid & 0x3) == 0) {
      ...
      }
      {
        ...
        func(frag_index_list, 1, mem_index + 8);
      }
    }
  } else {
    if (!(tid & 0b11100)) {
      ...
      }
      {
        ...
        func(frag_index_list, 1, mem_index + 1);
      }
      {
      ...
      }
      {
        ...
        func(frag_index_list, 1, mem_index + 9);
      }
    }
  }
}
  ...
  if (CORRECTION_TERMS == 3 || (lane_id & 0x1) == 0) {
    {
      ...
      frag_a.x[0 + 1] = detail::common::cast<half>(
          a_fp32 -
          detail::common::cast<float>(detail::common::cast<half>(a_fp32)));
    }
    {
      const auto a_fp32 = a_ptr[offset + 8];
      frag_a.x[2 + 1] = detail::common::cast<half>(
          a_fp32 -
          detail::common::cast<float>(detail::common::cast<half>(a_fp32)));
    }
    frag_a.x[8 + 1] = frag_a.x[0 + 1];
...
__device__ inline void make_direct_product_fragment(
...

  {
    ...
    if (is_residual) {
      frag_b.x[0] = detail::common::cast<half>(
          b_fp32 - detail::common::cast<float>(frag_b.x[0]));
    }
  }
  {
    ...
    if (is_residual) {
      frag_b.x[4] = detail::common::cast<half>(
          b_fp32 - detail::common::cast<float>(frag_b.x[4]));
    }
  }
  frag_b.x[8] = frag_b.x[0];
  frag_b.x[12] = frag_b.x[4];
