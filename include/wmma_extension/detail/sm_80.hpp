  ...
  if (layout == nvcuda::wmma::mem_col_major) {
    ...
    for (unsigned x = 0; x < frag.num_elements; x++) {
      const unsigned index =
          start_index + (x & 0b1) * 16 + (x & 0b10) * 4 + (x & 0b100) * 32;
    ...
    for (unsigned x = 0; x < frag.num_elements; x++) {
      const unsigned index =
          start_index + (x & 0b1) + (x & 0b10) * 64 + (x & 0b100) * 2;
      const unsigned frag_index_list[1] = {x};
      func(frag_index_list, 1, index);
    }
  }
  ...
  if (layout == nvcuda::wmma::mem_col_major) {
    ...
    if (load_flag) {
      {
        ...
        func(frag_index_list, 1, index_offset + 0);
      }
      {
    ...
    if (load_flag) {
      ...
      }
      {
        ...
        func(frag_index_list, 1, index_offset + 1);
      }
      {
      ...
      }
      {
        ...
        func(frag_index_list, 1, index_offset + 9);
      }
    }
  }
}
...
__device__ inline void make_direct_product_fragment(
  ...
  if ((CORRECTION_TERMS == 3) || ((lane_id & 0x1) == 0)) {
    frag_a.x[1] = frag_a.x[8 + 1] =
        common::cast<half>(a0 - common::cast<float>(frag_a.x[0]));
    frag_a.x[3] = frag_a.x[8 + 3] =
        common::cast<half>(a8 - common::cast<float>(frag_a.x[2]));
  }
}
