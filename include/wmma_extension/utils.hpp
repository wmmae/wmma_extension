...
__device__ __host__ inline
    typename mtk::wmma::detail::common::storage_t<DST_T>::type
    cast(const SRC_T v) {
...
__device__ inline void cp_async(void *const smem, const void *const gmem) {
  ...
  static_assert(Size == 4 || Size == 8 || Size == 16,
                "Size must be one of 4, 8 and 16");
  const unsigned smem_int_ptr = detail::get_smem_ptr_uint(smem);
  asm volatile(
      "{cp.async.ca.shared.global [%0], [%1], %2;}" ::"r"(smem_int_ptr),
      "l"(gmem), "n"(Size));
#else
  for (unsigned i = 0; i < Size / 4; i++) {
    *(reinterpret_cast<uint32_t *>(smem) + i) =
        *(reinterpret_cast<const uint32_t *>(gmem) + i);
  }
#endif
}
...
template <int N> __device__ inline void wait_group() {
#if __CUDA_ARCH__ >= 800
