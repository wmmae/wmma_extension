...
__device__ void fill_fragment(
    fragment_complex<Use, m, n, k, T, Layout, Policy> &frag,
    const typename mtk::wmma::detail::common::storage_t<
        typename mtk::wmma::tcec::detail::sub_frag_t<Use, T>::type>::type real,
    const typename mtk::wmma::detail::common::storage_t<
        typename mtk::wmma::tcec::detail::sub_frag_t<Use, T>::type>::type imag =
        0) {
...
template <class Use, int m, int n, int k, class T, class Layout, class Policy>
__device__ void
fill_zero(fragment_complex<Use, m, n, k, T, Layout, Policy> &frag) {
...
__device__ void load_matrix_sync(
    fragment_complex<nvcuda::wmma::accumulator, m, n, k, T, void,
                     mtk::wmma::tcec::Policy<Op, mtk::wmma::tcec::with_ec, fm,
                                             fn, fk>> &frag,
    const cuComplex *const ptr, const unsigned ldm, const bool sync = true) {
  using Policy =
      ...
      [&](const unsigned frag_index_list[], const unsigned frag_index_count,
        ...
        for (unsigned bm = 0; bm < frag.num_sub_frag_m; bm++) {
          for (unsigned bn = 0; bn < frag.num_sub_frag_n; bn++) {
            ...
            frag.real.sub_frag[bm + frag.num_sub_frag_m * bn].x[frag_index] =
                v.x;
            frag.imag.sub_frag[bm + frag.num_sub_frag_m * bn].x[frag_index] =
                v.y;
            frag.real.sub_d_frag[bm + frag.num_sub_frag_m * bn].x[frag_index] =
                0.f;
            frag.imag.sub_d_frag[bm + frag.num_sub_frag_m * bn].x[frag_index] =
                0.f;
          }
        }
      });
...
__device__ void load_matrix_sync(
    fragment_complex<nvcuda::wmma::accumulator, m, n, k, T, void,
                     mtk::wmma::tcec::Policy<Op, mtk::wmma::tcec::without_ec,
                                             fm, fn, fk>> &frag,
    const cuComplex *const ptr, const unsigned ldm, const bool sync = true) {
  using Policy =
      mtk::wmma::tcec::Policy<Op, mtk::wmma::tcec::without_ec, fm, fn, fk>;
  constexpr auto frag_m = mtk::wmma::tcec::detail::select_value<
      nvcuda::wmma::accumulator, Policy::m, Policy::k, Policy::m>::value;
  constexpr auto frag_n = mtk::wmma::tcec::detail::select_value<
      nvcuda::wmma::accumulator, Policy::k, Policy::n, Policy::n>::value;
...
template <int m, int n, int k, class T, class Op, class Ec, int fm, int fn,
          int fk>
__device__ void load_matrix_sync(
    fragment_complex<nvcuda::wmma::accumulator, m, n, k, T, void,
                     mtk::wmma::tcec::Policy<Op, Ec, fm, fn, fk>> &frag,
    const cuComplex *const ptr, const unsigned ldm,
...
template <class MatrixLayout, class Use, int m, int n, int k, class T,
          class Layout, class Op, int fm, int fn, int fk>
__device__ void load_matrix_sync(
    fragment_complex<Use, m, n, k, T, Layout,
                     mtk::wmma::tcec::Policy<Op, mtk::wmma::tcec::with_ec, fm,
                                             fn, fk>> &frag,
    const cuComplex *const ptr, const unsigned ldm, const bool sync = true) {
  using Policy =
  ...
  mtk::wmma::tcec::detail::foreach_ij_wrapper<
      Use, T, Layout, Policy>{}([&](const unsigned frag_index_list[],
                                    const unsigned frag_index_count,
                                    const unsigned i, const unsigned j) {
    for (unsigned bm = 0; bm < frag.num_sub_frag_m; bm++) {
      for (unsigned bn = 0; bn < frag.num_sub_frag_n; bn++) {
        ...
        for (unsigned bm = 0; bm < frag.num_sub_frag_m; bm++) {
          for (unsigned bn = 0; bn < frag.num_sub_frag_n; bn++) {
            ...
            const auto cv = ptr[mem_offset];
            {
              ...
              for (unsigned f = 0; f < frag_index_count; f++) {
                const auto frag_index = frag_index_list[f];
                frag.real.sub_frag[bm + frag.num_sub_frag_m * bn]
                    .x[frag_index] = hv;
              }
            }
            {
              ...
              for (unsigned f = 0; f < frag_index_count; f++) {
                const auto frag_index = frag_index_list[f];
                frag.imag.sub_frag[bm + frag.num_sub_frag_m * bn]
                    .x[frag_index] = hv;
              }
            }
          }
...
__device__ void load_matrix_sync_with_mul(
    fragment_complex<Use, m, n, k, T, Layout,
                     mtk::wmma::tcec::Policy<Op, mtk::wmma::tcec::with_ec, fm,
                                             fn, fk>> &frag,
    const cuComplex *const ptr, const unsigned ldm, const float mul,
...
__device__ void load_matrix_sync_with_mul(
    fragment_complex<Use, m, n, k, T, Layout,
                     mtk::wmma::tcec::Policy<Op, Ec, fm, fn, fk>> &frag,
    const float *const ptr, const unsigned ldm, const float mul,
    const bool sync = true) {
      ...
      [&](const unsigned frag_index_list[], const unsigned frag_index_count,
        ...
        for (unsigned bm = 0; bm < frag.num_sub_frag_m; bm++) {
          for (unsigned bn = 0; bn < frag.num_sub_frag_n; bn++) {
            ...
            const auto frag_index = frag_index_list[0];
            cuComplex c;
            c.x = (frag.real.sub_frag[bm + frag.num_sub_frag_m * bn]
                       .x[frag_index] +
                   detail::correction_scale_1<T>(
                       frag.real.sub_d_frag[bm + frag.num_sub_frag_m * bn]
                           .x[frag_index]));
            c.y = (frag.imag.sub_frag[bm + frag.num_sub_frag_m * bn]
                       .x[frag_index] +
                   detail::correction_scale_1<T>(
                       frag.imag.sub_d_frag[bm + frag.num_sub_frag_m * bn]
                           .x[frag_index]));
            ptr[mem_offset] = c;
          }
        }
...
template <int m, int n, int k, class A_Layout, class B_Layout, class T,
          class Op, class Ec, int fm, int fn, int fk>
__device__ void mma_sync(
    fragment_complex<nvcuda::wmma::accumulator, m, n, k, T, void,
                     mtk::wmma::tcec::Policy<Op, Ec, fm, fn, fk>> &frag_d,
    const fragment_complex<nvcuda::wmma::matrix_a, m, n, k, T, A_Layout,
                           mtk::wmma::tcec::Policy<Op, Ec, fm, fn, fk>> &frag_a,
    const fragment_complex<nvcuda::wmma::matrix_b, m, n, k, T, B_Layout,
                           mtk::wmma::tcec::Policy<Op, Ec, fm, fn, fk>> &frag_b,
    const fragment_complex<nvcuda::wmma::accumulator, m, n, k, T, void,
                           mtk::wmma::tcec::Policy<Op, Ec, fm, fn, fk>>
        &frag_c) {
  ...
  mtk::wmma::tcec::mma_sync(frag_d.real, frag_a.imag,
                            mtk::wmma::tcec::neg(frag_b.imag), frag_d.real);
}
...
__device__ void mma_rz_sync(
  ...
  mtk::wmma::tcec::mma_rz_sync(frag_d.imag, frag_a.imag, frag_b.real,
                               frag_c.imag);
  mtk::wmma::tcec::mma_rz_sync(frag_d.imag, frag_a.real, frag_b.imag,
                               frag_d.imag);
  mtk::wmma::tcec::mma_rz_sync(frag_d.real, frag_a.imag,
                               mtk::wmma::tcec::neg(frag_b.imag), frag_d.real);
}
...
__device__ void mma_rn_sync(
  ...
  mtk::wmma::tcec::mma_rn_sync(frag_d.imag, frag_a.imag, frag_b.real,
                               frag_c.imag);
  mtk::wmma::tcec::mma_rn_sync(frag_d.imag, frag_a.real, frag_b.imag,
                               frag_d.imag);
  mtk::wmma::tcec::mma_rn_sync(frag_d.real, frag_a.imag,
                               mtk::wmma::tcec::neg(frag_b.imag), frag_d.real);
}
