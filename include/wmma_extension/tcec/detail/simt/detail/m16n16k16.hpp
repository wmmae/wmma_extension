...
class fragment<nvcuda::wmma::matrix_a, 16, 16, 16, T, Layout>
    : public mtk::wmma::mma_simt::detail::__frag_base<T, 8> {};
template <class T, class Layout>
class fragment<nvcuda::wmma::matrix_b, 16, 16, 16, T, Layout>
    : public mtk::wmma::mma_simt::detail::__frag_base<T, 8> {};
template <class T>
class fragment<nvcuda::wmma::accumulator, 16, 16, 16, T>
    : public mtk::wmma::mma_simt::detail::__frag_base<T, 8> {};

// foreach
template <class Func, class T>
__device__ void foreach (fragment<nvcuda::wmma::matrix_a, 16, 16, 16, T,
  ...
  for (unsigned i = 0; i < frag.num_elements; i++) {
    {
      ...
      func(frag_index_list, 1, m + 16 * (n_offset + i));
    }
  }
}
...
template <class Func, class T>
__device__ void foreach (fragment<nvcuda::wmma::matrix_b, 16, 16, 16, T,
                                  nvcuda::wmma::col_major> &frag,
                         const Func func) {
  ...
  for (unsigned i = 0; i < frag.num_elements; i++) {
    {
      const unsigned frag_index_list[1] = {i};
      func(frag_index_list, 1, n * 16 + (m_offset + i));
    }
  }
}
...
__device__ inline void foreach (
    mtk::wmma::mma_simt::fragment<nvcuda::wmma::accumulator, 16, 16, 16, T>
        &frag,
    const nvcuda::wmma::layout_t layout, const Func func) {
  const auto n = threadIdx.x & 0xf;
  const auto m_offset = (mtk::wmma::detail::common::get_lane_id() >> 4) << 3;
...
// foreach_ij
template <class Func, class T>
__device__ void foreach_ij(fragment<nvcuda::wmma::matrix_a, 16, 16, 16, T,
                                    nvcuda::wmma::col_major> &frag,
                           const Func func) {
  const auto m = threadIdx.x & 0xf;
  const auto n_offset = (mtk::wmma::detail::common::get_lane_id() >> 4) << 3;
  for (unsigned i = 0; i < frag.num_elements; i++) {
    {
      const unsigned frag_index_list[1] = {i};
      func(frag_index_list, 1, m, n_offset + i);
    }
  }
}
...
template <class Func, class T>
__device__ void foreach_ij(fragment<nvcuda::wmma::matrix_b, 16, 16, 16, T,
                                    nvcuda::wmma::col_major> &frag,
                           const Func func) {
  const auto n = threadIdx.x & 0xf;
  const auto m_offset = (mtk::wmma::detail::common::get_lane_id() >> 4) << 3;
  for (unsigned i = 0; i < frag.num_elements; i++) {
    {
      const unsigned frag_index_list[1] = {i};
      func(frag_index_list, 1, m_offset + i, n);
    }
  }
}
...
foreach_ij(mtk::wmma::mma_simt::fragment<nvcuda::wmma::accumulator, 16, 16, 16,
                                         T> &frag,
           const nvcuda::wmma::layout_t layout, const Func func) {
  const auto n = threadIdx.x & 0xf;
  const auto m_offset = (mtk::wmma::detail::common::get_lane_id() >> 4) << 3;
...
// foreach_v
template <class Func, class T>
__device__ void foreach_v(fragment<nvcuda::wmma::matrix_a, 16, 16, 16, T,
                                   nvcuda::wmma::col_major> &frag,
                          const Func func) {
  const auto m = threadIdx.x & 0xf;
  if (threadIdx.x & 0b10000u)
    return;

  {
    const unsigned frag_index_list[1] = {0};
    func(frag_index_list, 1, m);
  }
}
...
__device__ void foreach_v(fragment<nvcuda::wmma::matrix_a, 16, 16, 16, T,
                                   nvcuda::wmma::row_major> &frag,
                          const Func func) {
  if (threadIdx.x & 0b01111u)
    return;
  const auto n_offset = (mtk::wmma::detail::common::get_lane_id() >> 4) << 3;
  for (unsigned i = 0; i < frag.num_elements; i++) {
    {
      const unsigned frag_index_list[1] = {i};
      func(frag_index_list, 1, n_offset + i);
    }
  }
}

template <class Func, class T>
__device__ void foreach_v(fragment<nvcuda::wmma::matrix_b, 16, 16, 16, T,
                                   nvcuda::wmma::col_major> &frag,
                          const Func func) {
  if (threadIdx.x & 0b01111u)
    return;
  ...
  for (unsigned i = 0; i < frag.num_elements; i++) {
    {
      const unsigned frag_index_list[1] = {i};
      func(frag_index_list, 1, m_offset + i);
    }
  }
}
...
__device__ void foreach_v(fragment<nvcuda::wmma::matrix_b, 16, 16, 16, T,
  ...
  {
    const unsigned frag_index_list[1] = {0};
    func(frag_index_list, 1, n);
  }
}

template <class Func, class T>
__device__ inline void
foreach_v(mtk::wmma::mma_simt::fragment<nvcuda::wmma::accumulator, 16, 16, 16,
                                        T> &frag,
          const nvcuda::wmma::layout_t layout, const Func func) {
  if (layout == nvcuda::wmma::mem_col_major) {
    if (threadIdx.x & 0b01111u)
...
__device__ void mma_sync(
    ...
    const mtk::wmma::mma_simt::detail::fma<AB_T, AB_T, C_T> &mfma =
        mtk::wmma::mma_simt::detail::fma<AB_T, AB_T, C_T>()) {
  ...
  constexpr unsigned swap_index_list[num_swaps] = {1, 2, 1, 4, 2, 1, 2, 8,
                                                   4, 2, 1, 2, 4, 2, 1};
  ...
  // collect C frag
  const auto offset_0 =
      (mtk::wmma::detail::common::get_lane_id() >> 4) * frag_c.num_elements;
  const auto offset_1 = frag_c.num_elements - offset_0;
  for (unsigned i = 0; i < frag_c.num_elements; i++) {
    frag_d.x[i] = array_acc[i + offset_0] +
                  __shfl_xor_sync(0xffffffff, array_acc[i + offset_1], 16) +
                  frag_c.x[i];
  }
}
