...
template <class Use, int m, int n, int k, class T, class Layout, int fm, int fn,
          int fk>
struct fragment<
    Use, m, n, k, T, Layout,
    mtk::wmma::tcec::Policy<mtk::wmma::tcec::op_simt,
                            mtk::wmma::tcec::without_ec, fm, fn, fk>> {
  ...
  for (unsigned bm = 0; bm < frag.num_sub_frag_m; bm++) {
    for (unsigned bn = 0; bn < frag.num_sub_frag_n; bn++) {
      mtk::wmma::tcec::detail::fill_fragment_wrapper<Use, sub_frag_t, Layout,
                                                     Policy, storage_t>{}(
          frag.sub_frag[bm + frag.num_sub_frag_m * bn], v);
    }
  }
}
  ...
  for (unsigned bm = 0; bm < frag.num_sub_frag_m; bm++) {
    for (unsigned bn = 0; bn < frag.num_sub_frag_n; bn++) {
      mtk::wmma::tcec::detail::fill_zero_wrapper<Use, sub_frag_t, Layout,
                                                 Policy>{}(
          frag.sub_frag[bm + frag.num_sub_frag_m * bn]);
    }
  }
}

// Load matrix
template <class MatrixLayout, int m, int n, int k, class T, int fm, int fn,
          int fk, class MEM_T>
...
template <class MatrixLayout, class Use, int m, int n, int k, class T,
          class Layout, int fm, int fn, int fk, class MEM_T>
__device__ void load_matrix_sync(
    fragment<Use, m, n, k, T, Layout,
             mtk::wmma::tcec::Policy<mtk::wmma::tcec::op_simt,
                                     mtk::wmma::tcec::without_ec, fm, fn, fk>>
        ...
        for (unsigned bm = 0; bm < frag.num_sub_frag_m; bm++) {
          for (unsigned bn = 0; bn < frag.num_sub_frag_n; bn++) {
            const auto mem_offset =
                mtk::wmma::tcec::detail::compute_mem_offset<frag_m, frag_n,
                                                            MatrixLayout>{}(
                    mem_index, ldm, bm * frag_m, bn * frag_n);
...
__device__ void store_matrix_sync(
  ...
  mtk::wmma::tcec::detail::foreach_wrapper<nvcuda::wmma::accumulator, T, void,
                                           Policy>{}(
          ...
          for (unsigned bn = 0; bn < frag.num_sub_frag_n; bn++) {
            const auto mem_offset =
                mtk::wmma::tcec::detail::compute_mem_offset<frag_m, frag_n,
                                                            MatrixLayout>{}(
                    mem_index, ldm, bm * frag_m, bn * frag_n);
            ptr[mem_offset] = frag.sub_frag[bm + frag.num_sub_frag_m * bn]
                                  .x[frag_index_list[0]];
          }
        }
      });
...
__device__ void load_vector(
    fragment<Use, m, n, k, T, Layout,
             mtk::wmma::tcec::Policy<mtk::wmma::tcec::op_simt,
                                     mtk::wmma::tcec::without_ec, fm, fn, fk>>
        &frag,
    const MEM_T *const ptr, const float mul) {
  using Policy =
      mtk::wmma::tcec::Policy<mtk::wmma::tcec::op_simt,
                              mtk::wmma::tcec::without_ec, fm, fn, fk>;
  constexpr auto frag_m =
...
// mma
template <int m, int n, int k, class A_Layout, class B_Layout, class AB_T,
          class CD_T, int fm, int fn, int fk>
__device__ void mma_sync(
    fragment<nvcuda::wmma::accumulator, m, n, k, CD_T, void,
             mtk::wmma::tcec::Policy<mtk::wmma::tcec::op_simt,
                                     mtk::wmma::tcec::without_ec, fm, fn, fk>>
        &frag_d,
    const fragment<nvcuda::wmma::matrix_a, m, n, k, AB_T, A_Layout,
                   mtk::wmma::tcec::Policy<mtk::wmma::tcec::op_simt,
                                           mtk::wmma::tcec::without_ec, fm, fn,
                                           fk>> &frag_a,
    const fragment<nvcuda::wmma::matrix_b, m, n, k, AB_T, B_Layout,
                   mtk::wmma::tcec::Policy<mtk::wmma::tcec::op_simt,
                                           mtk::wmma::tcec::without_ec, fm, fn,
                                           fk>> &frag_b,
    const fragment<nvcuda::wmma::accumulator, m, n, k, CD_T, void,
                   mtk::wmma::tcec::Policy<mtk::wmma::tcec::op_simt,
                                           mtk::wmma::tcec::without_ec, fm, fn,
                                           fk>> &frag_c) {
  using Policy =
  ...
  for (unsigned bm = 0; bm < num_m_block; bm++) {
    for (unsigned bn = 0; bn < num_n_block; bn++) {
      mtk::wmma::mma_simt::mma_sync(frag_d.sub_frag[bm + bn * num_m_block],
