...
__device__ inline void foreach (
    nvcuda::wmma::fragment<MatrixType, M, N, K, FT, MemMajor> &frag,
    Func func) {
  detail_namespace::foreach (frag, func);
}

template <class Frag_T, class Func> __device__ inline void foreach (Func func) {
  // Requirering `frag` as an argument does not look good but it can not be
  // helped because C++ does not support partial template specialization of a
  // templeta function. The `frag` below does not consume registers because of
  // optimization by nvcc. So this implementation is not a problem.
...
__device__ inline void foreach (const nvcuda::wmma::layout_t layout,
                                Func func) {
  // Requirering `frag` as an argument does not look good but it can not be
  // helped because C++ does not support partial template specialization of a
  // templeta function. The `frag` below does not consume registers because of
...
foreach_ij(nvcuda::wmma::fragment<MatrixType, M, N, K, FT, MemMajor> &frag,
           Func func) {
...
template <class Frag_T, class Func>
__device__ inline void foreach_ij(const nvcuda::wmma::layout_t layout,
                                  Func func) {
  // Requirering `frag` as an argument does not look good but it can not be
  // helped because C++ does not support partial template specialization of a
  // templeta function. The `frag` below does not consume registers because of
  // optimization by nvcc. So this implementation is not a problem.
  typename std::remove_reference<Frag_T>::type frag;
  detail_namespace::foreach_ij<typename Frag_T::element_type>(frag, layout,
                                                              func);
  __syncwarp();
}
...
__device__ inline void foreach_v(Func func) {
  ...
  // templeta function. The `frag` below does not consume registers because of
  // optimization by nvcc. So this implementation is not a problem.
  typename std::remove_const<typename std::remove_reference<Frag_T>::type>::type
      frag;
...
template <class Frag_T, class Func>
__device__ inline void foreach_v(const nvcuda::wmma::layout_t layout,
                                 Func func) {
  // Requirering `frag` as an argument does not look good but it can not be
  // helped because C++ does not support partial template specialization of a
  // templeta function. The `frag` below does not consume registers because of
...
__device__ inline void map(unsigned tid_list[2], unsigned fid_list[2],
                           unsigned &list_size, const unsigned i,
                           const unsigned j) {
  typename std::remove_const<typename std::remove_reference<Frag_T>::type>::type
      frag;
...
__device__ inline void
load_vector(nvcuda::wmma::fragment<Use, M, N, K, FT, Layout> &frag,
            const T *const ptr, const bool fill = true) {
  if (fill)
    mtk::wmma::fill_zero(frag);
  mtk::wmma::foreach_v<decltype(frag)>([&](const unsigned *frag_index_list,
                                           const unsigned fragment_index_count,
                                           const unsigned mem_index) {
    for (unsigned i = 0; i < fragment_index_count; i++)
      frag.x[frag_index_list[i]] = mtk::wmma::detail::common::cast<
          typename mtk::wmma::detail::common::storage_t<FT>::type>(
          ptr[mem_index]);
  });
}
...
__device__ inline void
load_vector(nvcuda::wmma::fragment<Use, M, N, K, FT, Layout> &frag,
            const T *const ptr, const T mul, const bool fill = true) {
  if (fill)
    mtk::wmma::fill_zero(frag);
  mtk::wmma::foreach_v<decltype(frag)>([&](const unsigned *frag_index_list,
                                           const unsigned fragment_index_count,
...
__device__ inline void
load_vector(nvcuda::wmma::fragment<Use, M, N, K, FT, Layout> &frag,
            const T *const ptr, const nvcuda::wmma::layout_t layout,
...
load_vector(nvcuda::wmma::fragment<Use, M, N, K, FT, Layout> &frag,
            const T *const ptr, const nvcuda::wmma::layout_t layout,
            const T mul, const bool fill = true) {
  if (fill)
    mtk::wmma::fill_zero(frag);
  mtk::wmma::foreach_v<decltype(frag)>(
      layout,
...
__device__ inline void store_vector(
    T *const ptr,
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, M, N, K, FT> &frag,
  ...
  mtk::wmma::foreach_v<decltype(frag)>(
      ...
      [&](const unsigned *frag_index_list, const unsigned fragment_index_count,
          const unsigned mem_index) {
        for (unsigned i = 0; i < fragment_index_count; i++)
          ptr[mem_index] = mtk::wmma::detail::common::cast<
              typename mtk::wmma::detail::common::storage_t<T>::type>(
              frag.x[frag_index_list[i]]);
      });
}
...
__device__ inline void store_vector(
    T *const ptr,
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, M, N, K, FT> &frag,
    const T mul, const nvcuda::wmma::layout_t layout) {
  mtk::wmma::foreach_v<decltype(frag)>(
      layout,
      [&](const unsigned *frag_index_list, const unsigned fragment_index_count,
          const unsigned mem_index) {
...
__device__ void
add_eye(nvcuda::wmma::fragment<nvcuda::wmma::accumulator, M, N, K, T> &frag,
        const FT a) {
...
__device__ inline void make_direct_product_fragment(
    ...
    const T *const x, const S *const dx, const bool fill = true) {
...
__device__ inline void make_direct_product_fragment(
    nvcuda::wmma::fragment<MatrixType, M, N, K, FT, MemMajor> &frag_x,
    const float *const x, const bool fill = true) {
