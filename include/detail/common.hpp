...
namespace detail {
namespace common {
template <class T> inline __device__ __host__ T cast(const float v);
template <class T> inline __device__ __host__ T cast(const half v);
template <> inline __device__ __host__ float cast(const float v){return v;}
template <> inline __device__ __host__ float cast(const half v){return __half2float(v);}
template <> inline __device__ __host__ half cast(const float v){return __float2half(v);}
template <> inline __device__ __host__ half cast(const half v){return v;}
