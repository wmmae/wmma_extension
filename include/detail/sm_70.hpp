...
template <>
__device__ inline void store_vector_sync<float>(float* const ptr, nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, float>& frag, const nvcuda::wmma::layout_t layout) {
	const auto tid = threadIdx.x & 0x1f;
	if (layout == nvcuda::wmma::mem_col_major) {
...
// partial specialization
template <>
__device__ inline void store_vector_sync<float>(float* const ptr, nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, float>& frag, const float mul, const nvcuda::wmma::layout_t layout) {
	const auto tid = threadIdx.x & 0x1f;
	if (layout == nvcuda::wmma::mem_col_major) {
		if (!(tid & 0b10) && !(tid & 0b1000)) {
			const auto mem_index = ((tid & 0b10000) >> 2) + (tid & 0b1) + ((tid & 0b100) << 1);
...
__device__ inline void make_identity_matrix(nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, T>& frag) {
	...
	if(p0 == 0 || p0 == 3) {
		frag.x[(warp_id & 0x3) + index_offset] = common::cast<T>(1.0f);
	}
	__syncthreads();
}
...
__device__ inline void make_direct_product_fragment(
	...
	const T* const is_residual = ((warp_id & 0x1) != 0);
	const unsigned a_offset = ((warp_id & 0x10) >> 2) + ((warp_id & 0x4) << 1);

#pragma unroll
	for (unsigned i = 0; i < 4; i++) {
		const a_fp32 = a_ptr[a_offset + i];
...
__device__ inline void make_direct_product_fragment(
	...
	const is_residual = ((warp_id & 0x2) == 0);
	const unsigned b_offset = ((warp_id & 0x10) >> 2) + (warp_id & 0x8);
#pragma unroll
	for (unsigned i = 0; i < 4; i++) {
		const b_fp32 = b_ptr[b_offset + i];
		frag_b.x[i] = detail::common::cast<half>(b_fp32);
		if (is_residual)
			frag_b.x[i] = detail::common::cast<half>(a_fp32 - detail::common::cast<float>(frag_b.x[i]));
	}
}
} // namespace sm_70
} // namespace detail
